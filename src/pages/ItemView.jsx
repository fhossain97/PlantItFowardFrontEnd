...
<div class="flex flex-col md:flex-row md:max-w-xl rounded-lg bg-white shadow-lg">
  <img class=" w-full h-150 md:h-auto object-cover md:w-100 rounded-t-lg md:rounded-none md:rounded-l-lg" src={item.images} alt={item.description}  />
  <div class="p-6 flex flex-col justify-start">
    <h5 class="text-black-900 text-xl font-medium mb-2">{item.name}</h5>
    <p class="text-black-700 text-base mb-4">
      {item.description}
    </p>
    <p class="text-black-900 font-medium">Genus: {item.genus}</p>
<p class="text-black-900 font-medium">Status: {item.status}</p>
<p class="text-black-900 font-medium">Quantity: {item.quantity}</p>
          <span>
  ...
  <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
    <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
      ...
      <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
        ...
        onClick={closeModal}
>Cancel </button>
        <button type="button" class="inline-block px-6 py-2.5 bg-red-500 text-white font-large text-xs leading-tight uppercase rounded-full shadow-md hover:bg-red-600 hover:shadow-lg focus:bg-red-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-700 active:shadow-lg transition duration-150 ease-in-out"
